...
2, 3, 4 e 1, respectivamente, para cada uma destas notas e mostre esta 
média acompanhada pela mensagem "Media: ". Se esta média for maior ou igual
a 7.0, imprima a mensagem "Aluno aprovado.". Se a média calculada for 
inferior a 5.0, imprima a mensagem "Aluno reprovado.". Se a média calculada
for um valor entre 5.0 e 6.9, inclusive estas, o programa deve imprimir a
...
apresente na última linha uma mensagem "Media final: " seguido da 
média final para esse aluno.
...
nota_um, nota_dois, nota_tres, nota_quatro = map(float, input().split())

media = ((nota_um * 2) + (nota_dois * 3) + (nota_tres * 4) + (nota_quatro)) / 10
...
if 10.0 > media >= 7.0:
    ...
    remedia = (media + recuperacao) / 2
    if 10.0 > remedia >= 5.0:
        print(f"Aluno aprovado.")
        print(f"Media final: {remedia:.1f}")
    if 0 < remedia < 5.0:
